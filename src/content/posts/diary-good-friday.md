...
如图所示，把原始公式左边的字母放在金字塔尖（当然你也可以把它想象成 $\color{brown}\text{答辩}$）。把右边的量放在下面，有几个分几格。这时我们要求能求任一量的变形公式，只需把那个量领出来，把它和金字塔用等号连起来起来，把金字塔上下层的分割线当作分数线（是分数的分数线，不是成绩的分数线哦），下面的隔板去掉，架子去掉就可以啦~
